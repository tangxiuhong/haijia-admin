<template>
  <div>
      ...
      <el-row>
        ...
        <el-col :span="12" align="right">
          <el-form :inline="true" :model="formInline" class="demo-form-inline" @submit.native.prevent>
            <el-form-item>
              <el-input v-model="formInline.name" clearable  placeholder="请输入机型名称" v-on:keyup.enter.native="onQuery"/>
            </el-form-item>
            <el-form-item>
    ...
    <!--新增编辑弹框-->
    <machine-model-dlog :show-dlog="showDlog" :dialog-title="dialogTitle" :log-id="logId" @onMachineModelDlog="getData"></machine-model-dlog>
  </div>
</template>

<script>
import machineModelDlog from './components/machine-model-dlog';
import { machineModelDelete, machineModelList, moduleAxios } from '../../../api/haijia/machine'
export default {
  name: "machine",
  components: {
    machineModelDlog
  ...
  methods: {
    ...
    onDeleteData(row) {
      this.$confirm('确定删除?', '提示', {
        ...
        machineModelDelete({id: row.id}).then(res => {
          if (res.data.status == 200) {
            this.getTableData()
            this.$message({
              type: 'success',
